...
.fd-line { display: flex; gap: 10px; padding: 0 10px; font: var(--dsw-font-markdown-code-block, 12px/1.7 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace); }
...
/* Highlighted rows: one shared CodeBlock per same-kind run of a patch — the same
   Shiki surface the sidebar's file preview renders through, so a line of code
   carries the same colors in both places. The primitive's line-numbered layout is
   what makes a diff background possible: it turns every line span into a block
   element, so the run's own class paints the +/- tint per line while the tokens
   stay intact. Its banner (grammar name + copy) is noise on a diff, and its
   counter would restart at every run, so the banner is hidden and the gutter
   carries this view's +/- sign instead. word-break and wrapping stay at pre, like
   the sidebar, with the pre scrolling a long line horizontally rather than
   reflowing it. */
.fd-code .fd-code-block { --dsl-code-block-background: transparent; --dsl-code-block-border-radius: 0; --dsl-code-block-line-white-space: pre; margin: 0; }
.fd-code .fd-code-block [data-code-block-banner] { display: none; }
.fd-code .fd-code-block pre { padding: 0; background: transparent; }
.fd-code .fd-code-block pre code > .line::before { content: ''; }
.fd-code.fd-code-add .fd-code-block pre code > .line { background: color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent); }
.fd-code.fd-code-del .fd-code-block pre code > .line { background: color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent); }
.fd-code.fd-code-add .fd-code-block pre code > .line::before { content: '+'; color: var(--dsw-alias-state-success-primary); }
.fd-code.fd-code-del .fd-code-block pre code > .line::before { content: '-'; color: var(--dsw-alias-state-error-primary); }
...
const plugin = {
  ...
  apply(ctx) {
    ...
    // The harness's own client UI primitives, reached through the shell's static
    ...
    // `dsh.client.external` graph edge. Two entries are used here:
    //   * `MarkdownText` — the GFM+KaTeX rendering the Chat tab gives assistant
    //     answers;
    //   * `CodeBlock` — the Shiki code surface the sidebar's file preview renders
    //     through (documentpreview's `CodeBody` -> `primitives.CodeBlock`), which
    //     is what lets a diff row show the same colors for the same source.
    // Guarded because a harness that drops the entry, or the dynamic payload where
    // `require` does not exist at all, must degrade to pre-wrapped plain text and
    // plain diff rows rather than take the whole view down.
    const primitives = (() => {
      try {
        const loaded = require('@deepseek-ai/dsh-client-ui-primitives')
        return loaded === null || loaded === undefined ? null : loaded
      } catch (error) {
        return null
      }
    })()
    ...
    const MarkdownText = primitive('MarkdownText')
    const CodeBlock = primitive('CodeBlock')

    // __fd-answer-begin
    /**
     * The Markdown chrome the harness renderer requires, in this plugin's UI
     * language. `MarkdownText` reads `labels.code.copyLabel`/`copiedLabel` for a
     * fenced block's copy control and `labels.footnotes` for the footnote heading —
     * all without a guard, so an answer containing a code fence or a footnote
     * throws `Cannot read properties of undefined (reading 'code')` when `labels` is
     * omitted. The Chat tab ships the same object (`markdownLabels(t)`).
     */
    ...
    /**
     * A render-time failure in the harness renderer must cost the Markdown, not the
     * tab: React unmounts the tree on an uncaught render error, which is how a
     * missing `labels` object blanked the whole Diff view. The boundary catches
     * render errors only (not event handlers or effects) and shows the same
     * pre-wrapped text as the no-renderer case. Keyed by `text`, so a message paged
     * in later retries Markdown after an earlier preview failed.
     */
     ...
     */
    const AnswerText = ({ text, className }) => {
      if (MarkdownText === null) {
        return React.createElement('div', { className: `${className} fd-answer-plain` }, text)
      }
      return React.createElement('div', { className: `${className} fd-answer-markdown` },
        React.createElement(MarkdownBoundary, {
          key: text,
          fallback: React.createElement('div', { className: 'fd-answer-plain' }, text),
        }, React.createElement(MarkdownText, { text, labels: MARKDOWN_LABELS })))
    }
    // __fd-answer-end
    ...
    // __fd-code-begin
    // The extension table the sidebar's code preview selects a grammar with
    // (documentpreview's `languageForPath`, which is not exported to plugins), so
    // a file is colored here exactly as it is there. The shared CodeBlock takes a
    // Shiki grammar id, not a filename; unknown suffixes stay plain.
    ...
    const languageForPath = (path) => {
      ...
      return LANGUAGE_BY_EXTENSION.get(match[1].toLowerCase())
    }

    // One plain patch row: the surface this view has always drawn, and the
    // fallback whenever the shared code primitive (or a grammar for the file) is
    // not available.
    const plainRow = (row, index) => React.createElement('div', { key: index, className: `fd-line fd-line-${row.kind}` },
      React.createElement('span', { className: 'fd-sign' }, row.kind === 'add' ? '+' : row.kind === 'del' ? '-' : ' '),
      React.createElement('span', { className: 'fd-text' }, row.text))

    // Consecutive rows of one kind go through a single CodeBlock. Shiki
    // highlights a document, not a line, so a whole change run is the smallest
    // unit that keeps multi-line constructs (block comments, template literals)
    // in context — while the instance count stays near the hunk count instead of
    // the line count. The run's kind becomes the wrapper's class.
    const rowGroups = (rows) => {
      const groups = []
      for (const row of rows) {
        const last = groups[groups.length - 1]
        if (last !== undefined && last.kind === row.kind) last.lines.push(row.text)
        else groups.push({ kind: row.kind, lines: [row.text] })
      }
      return groups
    }

    const RowLines = ({ rows, path }) => {
      const lang = CodeBlock === null ? undefined : languageForPath(path)
      if (lang === undefined) {
        return React.createElement('div', { className: 'fd-hunk' }, ...rows.map(plainRow))
      }
      return React.createElement('div', { className: 'fd-hunk' },
        ...rowGroups(rows).map((group, index) => React.createElement('div', { key: index, className: `fd-code fd-code-${group.kind}` },
          React.createElement(CodeBlock, {
            className: 'fd-code-block',
            code: group.lines.join('\n'),
            lang,
            // A diff run is whole; the streaming path is for a live tool card.
            streaming: false,
            // Only the line layout this view needs: the primitive's numbered
            // variant is what turns each `.line` into a block, which is what lets
            // the stylesheet paint a per-line diff background under the tokens.
            lineNumbers: true,
          }))))
    }
    // __fd-code-end

    const Hunk = ({ hunk, path }) => React.createElement(RowLines, { rows: hunkLines(hunk.oldText, hunk.newText), path })
    ...
    const CumulativeBody = ({ before, after, path }) => {
      ...
      return React.createElement(RowLines, { rows: diff.rows, path })
    }
    ...
    const ChangeEntry = ({ entry, index, ordinal, total }) => {
      ...
      return React.createElement(React.Fragment, null, header, React.createElement(Hunk, { hunk: entry, path: entry.path }))
    }
    ...
    const FileTotalRow = ({ agg }) => {
      ...
      return React.createElement('div', { className: 'fd-total-body' },
        header,
        React.createElement(CumulativeBody, { before: agg.before, after: agg.after, path: agg.path }))
    }
    ...
    const FileGroup = ({ path, group }) => {
      ...
      const body = []
      if (plan.ok) {
        body.push(React.createElement(CumulativeBody, { key: 'total', before: plan.states[0], after: plan.states[plan.states.length - 1], path }))
        if (plan.diverged) {
        ...
        body.push(React.createElement('div', { key: 'detail', className: 'fd-inset' },
          ...
          showDetail ? group.hunks.map((hunk, index) => React.createElement(Hunk, { key: index, hunk, path })) : null))
      } else {
        if (plan.reason !== '') {
          body.push(React.createElement('div', { key: 'reason', className: 'fd-reason' }, plan.reason))
        }
        body.push(React.createElement('div', { key: 'hunks', className: 'fd-inset' }, ...group.hunks.map((hunk, index) => React.createElement(Hunk, { key: index, hunk, path }))))
      }
